...
[시드 로드] → [수집: YouTube API / ScrapeCreators(IG·TT)] → [정규화·중복제거(channel+channel_id upsert)]
  → [등급 자동 분류] → [활성도 스코어링(전주 대비)] → [주간 스냅샷 저장]

시드: app/data/seed_channels.json
로컬/수동 트리거: POST /api/admin/pipeline/run
⚠️ contact_email은 수집하지 않는다 — 법무 검토(개인정보) 완료 전 미수집 방침.
주간 크레딧 비용: IG 1/명 + TT 2/명 (ScrapeCreators), YouTube는 자체 쿼터(무료).
"""
...
from .collectors.scrapecreators_collector import collect_instagram_stats, collect_tiktok_stats
...
CONCURRENCY = 5  # ScrapeCreators 동시 호출 제한
...
async def _collect_youtube(seeds: list[dict]) -> dict[str, dict]:
    """YouTube 배치 수집 → channel_id 기준 stats dict."""
    stats_map = {
        s["channel_id"]: s
        for s in await collect_channel_stats([s["channel_id"] for s in seeds])
    }
    results = {}
    for seed in seeds:
        ...
        results[seed["channel_id"]] = {
            "name": stat["name"],
            "followers": stat["followers"],
            "monthly_views": recent["monthly_views"],
            "engagement_rate": recent["engagement_rate"],
        }
    return results


async def _collect_scrapecreators(seeds: list[dict]) -> dict[str, dict]:
    """IG/TT 동시 수집 (세마포어 제한) → channel_id 기준 stats dict."""
    sem = asyncio.Semaphore(CONCURRENCY)

    async def one(seed):
        async with sem:
            fn = collect_instagram_stats if seed["channel"] == "instagram" else collect_tiktok_stats
            return seed["channel_id"], await fn(seed["handle"])

    results = {}
    pairs = await asyncio.gather(*(one(s) for s in seeds))
    for channel_id, stat in pairs:
        if stat:
            results[channel_id] = stat
    return results


async def run_weekly_pipeline() -> dict:
    """시드 전 채널 실수집 → 인플루언서 upsert + 주간 스냅샷 기록."""
    ...
    try:
        seeds = load_seeds()
        yt = [s for s in seeds if s["channel"] == "youtube"]
        sc = [s for s in seeds if s["channel"] in ("instagram", "tiktok")]

        stats: dict[str, dict] = {}
        stats.update(await _collect_youtube(yt))
        stats.update(await _collect_scrapecreators(sc))
        ...
        async with async_session() as db:
            ...
            for seed in seeds:
                stat = stats.get(seed["channel_id"])
                if not stat:
                    failed += 1
                    logger.warning(f"수집 실패 스킵: [{seed['channel']}] {seed['name']}")
                    continue
                ...
                if inf is None:
                    ...
                    updated += 1
                per_channel[seed["channel"]] = per_channel.get(seed["channel"], 0) + 1
                ...
                snap.monthly_views = stat["monthly_views"]
                snap.engagement_rate = stat["engagement_rate"]
                snap.growth_rate = growth
                snap.is_trending = is_trending(growth, stat["engagement_rate"])
